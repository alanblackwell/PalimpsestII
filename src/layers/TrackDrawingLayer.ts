...
// Replacement button defs for TrackDrawingLayer: [Rect][Ellipse][Path]
const REP_DEFS = [
...
export class TrackDrawingLayer extends MaskLayer implements PointSource {
  ...
  // Set by renderSlots() so renderOverlay() can place the smooth slider
  ...
  private _renderReplaceBtns(ctx: Ctx2D): void {
    ...
    this._repPathBounds    = { x: layout[2]!.x, y: layout[2]!.y, width: layout[2]!.w, height: TRK_BTN_H }
    for (const { x, y, w, label } of layout) renderTrackRepBtn(ctx, x, y, w, label)
  }

  // ── Interaction ───────────────────────────────────────────────────────

  // MaskLayer claims canvas hits for painting; defer to it unless the pointer
  // is on our own controls.
  protected override hitTestSelf(pt: Point): this | null {
    ...
    for (const bb of [this._repRectBounds, this._repEllipseBounds, this._repPathBounds]) {
