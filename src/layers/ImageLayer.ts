...
export class ImageLayer extends Layer implements ImageSource {
  ...
  // the shared math and full rationale. A manually positioned/scaled image
  // keeps the same proportion of the letterbox's short side, and the same
  // proportional offset from centre, that it had relative to the viewport
  // it was saved at. An image that was never manually moved (manualPosition
  // still null) needs no adjustment — recompute()'s viewport-centre
  // fallback already re-centres it on the new viewport on its own.
