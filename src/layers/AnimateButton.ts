// Shared layout, rendering, and hit-testing for the "Animate" convenience
// button shown on shape layers (RectLayer, EllipseLayer, PathLayer,
// StrokeLayer) when they are selected. Drawn via renderOverlay (unclipped).
...
const BTN_COLOUR = '#cf7ecf'   // Point/AnimPath accent
...
export function animateButtonHitTest(point: Point, done: boolean): boolean {
...
export function renderAnimateButton(ctx: Ctx2D, done: boolean): void {
  if (done) return
  const { x, y } = btnRect()
  ...
  ctx.fillText('Animate', x + 10, midY)
  ctx.restore()
