...
export class LayerStackWidget {
  ...
  // Candidates for the hotspot ratio: on-stack layers minus permanent chrome
  // (Root/Menu/Deletion — see Layer.hotspotExempt). Root and Menu do
  // essentially no work in recompute() by design, so counting them makes
  // any real content layer look artificially dominant purely because
  // there's nothing but near-zero-cost padding to compare it against; e.g.
  // one lone content layer alongside those three reads as "100% of the
  // stack's cost" immediately, which is a measurement artifact, not a
  // performer-relevant hotspot.
  private _hotspotCandidates(): Layer[] {
    return this._layers.filter(l => !l.hotspotExempt)
  }
  ...
  private _updateDropIndex(mouseY: number): void {
    ...
    // Index 0 is Root's slot and index 1 is DeletionLayer's — both are
    // permanently fixed (see moveUp/moveDown) and never valid drop targets.
    let best = 2, bestDist = Infinity
    for (let i = 2; i < this._layers.length; i++) {
  ...
  private _commitDrop(): void {
    ...
    if (from <= 1) return   // Root and DeletionLayer never move
    const to = Math.max(2, Math.min(this._layers.length - 1, this._dropIndex))
    if (from === to) return
